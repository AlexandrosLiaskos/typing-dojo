...
let _globalLoader: import('./content-loader').ContentLoader | null = null;

export function initializeContentLoader(storageRoot: string): void {
    const { ContentLoader } = require('./content-loader');
    _globalLoader = new ContentLoader(storageRoot);
    _globalLoader!.initialize();
}

export function getRandomContent(request: ContentRequest): TrainingContent {
    if (_globalLoader) {
        return _globalLoader.getRandomContent(request);
    }

    const { category, language, length } = request;
    const byCategory = contentLibrary.filter((c) => c.category === category);
    ...
    const pool = filtered.length > 0 ? filtered : byCategory.length > 0 ? byCategory : contentLibrary;
    return pool[Math.floor(Math.random() * pool.length)];
}

export function getAllContent(): TrainingContent[] {
    if (_globalLoader) {
        return _globalLoader.getAllContent();
    }
